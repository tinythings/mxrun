...
impl<'a> ResultMirror<'a> {
    ...
    fn resolve_linux_label(&self) -> Result<String, String> {
        self.kernel_major().and_then(|kernel| {
            self.libc_label()
                .map(|libc| format!("Linux {kernel} {libc}"))
        })
    }
    ...
    fn libc_label(&self) -> Result<String, String> {
        self.glibc_label().or_else(|_| self.musl_label())
    }

    fn glibc_label(&self) -> Result<String, String> {
        self.capture("getconf", &["GNU_LIBC_VERSION"])
            .and_then(|value| {
                value
                    .strip_prefix("glibc ")
    ...
    fn mirror_local_path(
        ...
        self.command("rsync", self.local_rsync_args(mirror_target, path))
            .status(log_path)
            .and_then(Self::ensure_success)
    }

    fn mirror_remote_path(
        &self,
        mirror_target: &MirrorTarget,
        path: &Path,
        log_path: &Path,
    ) -> Result<(), String> {
        self.command("rsync", self.remote_rsync_args(mirror_target, path))
            .status(log_path)
            .and_then(Self::ensure_success)
    }
    ...
    fn remote_manifest_path(&self) -> String {
        format!(
            ...
            self.mirror_plan.manifest().display()
        )
    }
